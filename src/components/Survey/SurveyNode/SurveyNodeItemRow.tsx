...
interface TSurveyNodeItemRowProps extends StackOwnProps {
  className?: string;
  id?: number | string;
...
export const SurveyNodeItemRow: React.FC<TSurveyNodeItemRowProps> = (props) => {
  ...
  return (
    <Stack
      // prettier-ignore
      className={classNames(className, 'SurveyNodeItemRow')}
      data-id={id}
      direction={responsiveNodeStackDirection}
      {...restProps}
    >
      <Box
        className={classNames('SurveyNodeItemRow_Title')}
        px={1}
        py={1}
      ...
      <Box
        // prettier-ignore
        className={classNames('SurveyNodeItemRow_Content')}
        sx={{ overflow: 'hidden' }}
        flex={1}
      >
        {children}

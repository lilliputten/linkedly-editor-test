...
import { mainRoute, mainCampaignRoute } from 'src/routes/appUrls';

import styles from './MainCampaignsList.module.scss';

export const MainCampaignItem: React.FC<{ item: TCampaignListItem }> = ({ item }) => {
  const { id, name } = item;
  const url = makeRootUrl([mainRoute, mainCampaignRoute, String(id)]);
  const imageUrl = getRandomStubImageUrl(id);
  const title = name || `Campaign ${id}`; // + (isDev ? ` (${imageUrl})` : '');
  return (
    <Card key={id} className="MainCampaignsListList-Item">
...
export const MainCampaignsList: React.FC<TPropsWithClassName> = observer((props) => {
  const { className } = props;
  useCommonAppNavigation();
  const isLogged = useLogged();
  const [ready, setReady] = React.useState(false);
  const [campaignsList, setCampaignsList] = React.useState<TCampaignListItem[]>([]);
  React.useEffect(() => {
    if (!isLogged && !isDev) {
      return;
    }
    // console.log('[MainCampaignsList] start fetch');
    fetch('/api/campaigns/list')
      .then((response) => response.json())
      .then((list: TCampaignListItem[]) => {
    ...
    <>
      <Scrollable className={classNames(className, styles.root)}>
        <Container maxWidth="md">
          <PageTitle mb={3}>Available campaigns list</PageTitle>
          <Stack className="MainCampaignsListList-List" spacing={2} useFlexGap>
            {/*
            <p>MainCampaignsList</p>
            <pre>{JSON.stringify(campaignsList, null, 2)}</pre>
            */}
            {campaignsList.map((item) => (
              <MainCampaignItem key={item.id} item={item} />
            ))}
          </Stack>
        </Container>
      </Scrollable>
